defmodule HaberdashWeb.ProductAccessoriesController do
  ...
  def create(conn, %{"product_id" => product_id, "accessories_id" => accessories_id}) do
    ...
    with {:ok, %ProductAccessories{} = product_accessories} <- Assoc.create_product_accessories(%{product_id: product_id, accessories_id: accessories_id}) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"product_id" => product_id, "accessories_id" => accessories_id}) do
    with {:ok, product_accessories} <- Assoc.get_product_accessories_by(%{accessories_id: accessories_id, product_id: product_id}) do
      ...
      {:error, :not_found}
        -> send_resp(conn, :not_found, Poison.encode!(%{message: "No accessory or product exist within the association", code: :not_found}))
    end
  end
  ...
  def delete(conn, %{"product_id" => product_id, "accessories_id" => accessories_id}) do
    ...
    with {:ok, product_accessories} = Assoc.get_product_accessories_by([product_id: product_id, accessories_id: accessories_id]),
    {:ok, %ProductAccessories{}} <- Assoc.delete_product_accessories(product_accessories) do
      send_resp(conn, :no_content, "")
    else
      {:error, :not_found} ->
        send_resp(conn, :not_found, Poison.encode!(%{message: "No accessory or product exist within this association.", code: :not_found}))
    end

defmodule Haberdash.OrdersWorkerTest do
  use ExUnit.Case
  use Haberdash.DataCase
  require Logger
  alias Haberdash.Transactions.{OrdersWorker, OrderRegistry, OrderSupervisor}
  ...
  defp create_order(%{product: product, accessories: accessories, franchise: franchise}) do
    ...
    {:ok, orders} = OrdersWorker.create_order(pid, %{items: [%{id: "prod_" <> product.id, accessories: [%{id: "acc_" <> accessories.id}]}]})
  ...
  describe "checking reduce map function for map_helpers macro" do
    ...
    test "reduce a nested order total to a single price", %{orders: orders, product: product, accessories: accessories, pid: pid} do
      price = OrdersWorker.reduce(orders, 0,
      fn key_val, acc ->
        case key_val do
          {"price", v} = tup ->
              v + acc
      ...
      {:ok, new_orders} = OrdersWorker.append_order(pid, orders["id"], %{items: [%{id: "prod_" <> new_product.id}]})
      new_price = OrdersWorker.reduce(new_orders, 0,
      fn key_val, acc ->
        case key_val do
          {"price", v} ->
              v + acc
